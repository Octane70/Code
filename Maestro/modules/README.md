...
This Python class supports Pololu's Maestro servo controller over USB serial or UART serial. Great for use with the Raspberry Pi.
...
Pololu's Maestro Windows installer sets up the Maestro Control Center, used to configure, test and program the controller.  Be sure the Maestro is configured for "USB Dual Port" for USB serial mode or "UART auto baudrate" for UART serial mode.  I believe the controller is setup for "USB Dual Port" serial mode by default.
